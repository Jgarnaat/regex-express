...
```
    ...
    parameters. So lets look at an example. Given the previous list if we wanted to find all days that were either Monday or Wednesday we would be able to do this with the '|' metaCharacter. Our Expression for this one is pretty simple, we make two capture groups and put the '|' between them (Monday)|(Wednesday) this will check the list and look for either the Monday or Wednesday words and return them.
        ...
        Now using our newfound regex knowledge lets take a look at a slightly more complicated expression and break it down. This regex is designed to look for certain urls
    /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/
    This looks intimidating but can be easily broken down to determine what exactly it is looking for. we see the first capture group has the ^  so this indicates that it will only return the           (https?:\/\/)?([\da-z\.-]+) capture group if it at the beginning of the url. the 'https?' is checking to see if https occurs one or more times followed by a ':' then it checks to see if it is followed by two // but because that is also a metaCharacter we have to put the \ in front to tell the expression that it indeed looking for iterations of /. and putting the ? after this capture group indicates that we are looking this entire group one or more times. 
        The next capture group we are looking at is the  ([\da-z\.-]+)\. group. This group will look first for any digit followed by and letter a-z followed by a '.' or a '-' and it will check that this 
    occurs one or more times followed by a '.'. The next capture group ([a-z\.]{2,6}) will check to for any letter between a-z followed by a '.' and the {2,6} indicates we are looking for between 2 and 6 times. The last capture group ([\/\w \.-]*)*\/?$/ is checking for a '/' followed by a word then a '.' and a dash, which will be checked for 0 or more times indicated by the * metaCharacter. and lastly it checks for a '/' to occur one or none times at the end of the query.
